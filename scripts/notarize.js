...
// notarize wrapper (which has been buggy across the 24.x/26.x line) and calls
// @electron/notarize directly with full options. Skipped on non-mac builds and
// when Apple credentials aren't set (local dev / unsigned CI).
...
exports.default = async function notarizing(context) {
  ...
  // Staple the ticket into the .app so Gatekeeper can verify offline. notarytool
  // does not staple — @electron/notarize only submits and polls. Staple failures
  // are logged but non-fatal: the app is still validly notarised, the ticket is
  // just fetched online instead of embedded.
  try {
