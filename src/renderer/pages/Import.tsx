...
export function Import() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold font-serif text-primary-800">Import Surveys</h1>
      ...
      <Card className="p-8">
        <div
          ...
          className={`
            ...
            ${isDragActive ? 'border-primary-300 bg-primary-50' : 'border-primary-200 hover:border-primary-300'}
          `}
        >
          <input {...getInputProps()} />
          <Upload className="w-12 h-12 mx-auto text-primary-500 mb-4" />
      ...
      {files.length > 0 && (
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-medium font-serif text-primary-800">
                      ...
                      <div className="flex items-center gap-2">
                        {result.status === 'success' && (
                          <>
                            ...
                            <span className="text-sm text-success-500">Imported</span>
                          </>
                        )}
                        {result.status === 'duplicate' && (
                          <>
                            <AlertCircle className="w-5 h-5 text-warning-500" />
                            <span className="text-sm text-warning-500">Duplicate</span>
                          </>
                        )}
                        {result.status === 'failed' && (
                          <>
                            <XCircle className="w-5 h-5 text-error-500" />
                            <span className="text-sm text-error-500">Failed</span>
                          </>
                        )}
                      </div>
                    )}
      ...
      {summary && (
        <Card className="p-6">
          <h2 className="text-lg font-medium font-serif text-primary-800 mb-4">Import Summary</h2>

          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="text-center">
              <p className="text-2xl font-semibold text-success-500">{summary.success}</p>
              <p className="text-sm text-primary-600">Imported</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-semibold text-warning-500">{summary.duplicates}</p>
              <p className="text-sm text-primary-600">Duplicates</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-semibold text-error-500">{summary.failed}</p>
          ...
          {results.length > 0 && (
            <div className="border-t pt-4">
              <h3 className="text-sm font-medium text-primary-800 mb-2">Details</h3>
